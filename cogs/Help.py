...
def format_help_page(bot, cog_name, curr_page, max_page):
    ...
    for command in lst_commands:
        embed.add_field(name=command.name, value=command.__doc__, inline=False)
        if isinstance(command, commands.core.Group):
            for subcommand in command.commands:
                embed.add_field(name=subcommand.qualified_name, value=subcommand.__doc__, inline=False)
    return embed
